...
contract('ExampleElection', function(accounts) {
	...
	it('delegated voting', function() {
		...
		return Election.deployed().then(function(instance) {
			...
			return inst.voteFor(accounts[5], 'Pinuzzo', {from: votingAccount});
		}).then(assert.fail).catch(function(error) {
			assert(error.message.indexOf('revert') >= 0, 'cannot vote for an account not allowed');
			return inst.voteFor.call(fromAccount, 'Pinuzzo', {from: votingAccount});
		}).then(function(success) {
			assert.equal(success, true, 'voteFor should succeed')
			return inst.voteFor(fromAccount, 'Pinuzzo', {from: votingAccount});
		}).then(function(receipt) {
			return inst.parties('Pinuzzo');
		}).then(function(votes) {
